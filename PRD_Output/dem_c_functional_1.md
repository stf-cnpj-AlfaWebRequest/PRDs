...
Tabela 4 — Campos de saída do sistema, com descrição funcional e exemplos de valores.
...
Identificadores recebidos de fonte externa (ex.: DB2MCI.CLIENTE, DAF, legacy) devem ser preservados exatamente como recebidos. Nenhuma transformação estrutural (incluindo conversão para formato alfanumérico) é permitida.
...
Filtro de autorização (FLTR-AUTZ) deve ser preservado em todas as transformações de identificador.

### RN08 — Contagem de Rodapé Precisa

Registro de rodapé (trailer) deve refletir contagem exata de registros processados, independentemente de transformações de identificador.
...
1. Primeira invocação: consultar DFE, armazenar resultado em cache (Redis, VSAM ou batch)
...
| **DB2MCI** | Tabela de clientes corporativos (CLIENTE); origem externa. CNPJ de fonte externa não deve ser transformado. | CNPJ (forma como recebido), dados de cliente | Correlação incorreta entre portador e cliente; perda de rastreabilidade de origem |
...
| **Subsistema DAF** | Integração seletiva para dados complementares de cartão. Consulta controlada por parâmetro de execução. | Chave de CNPJ (busca), dados de cartão extendido | Cartões sem dados complementares; incompletude de output quando DAF estava disponível |
...
| **Arquivos de Saída (Output)** | Geração de arquivos padronizados com dados enriquecidos. Consumidos por downstream. | Valor de CNPJ (sempre), dados cadastrais e metadados | Downstream recebe dados incompletos ou com identificadores ambíguos |
...
- **3.1 Classificação de Identificador**: Determinar se campo de pessoa jurídica é Chave de CNPJ ou Valor de CNPJ conforme contexto e nível de inferência
- **3.2 Validação (se Valor de CNPJ)**: Invocar SBDIGITO para validar dígito verificador
- **3.3 Busca em DB2VIP**: Consultar tabelas primárias usando Chave de CNPJ (ou converter antes de buscar)
- **3.4 Busca em DB2VIPA**: Consultar tabelas de suporte para enriquecimento (restrições, agências)
- **3.5 Validação de DB2MCI**: Se necessário, consultar dados de cliente externos (CNPJ preservado conforme origem)
- **3.6 Integração DAF (Condicional)**: Se parâmetro permite, invocar subsistema DAF para dados complementares
- **3.7 Enriquecimento de Dados**: Consolidar informações de múltiplas fontes, preservando integridade semântica de CNPJ
- **3.8 Classificação NR/RC**: Aplicar classificação de nível de risco e risco de crédito
- **3.9 Filtro FLTR-AUTZ**: Aplicar filtro de autorização conforme regras de negócio
- **3.10 Preparação de Saída**: Formatar registro de detalhe com Valor de CNPJ exposto (nunca Chave)
- **3.11 Escrita em Output**: Gravar registro em arquivo de saída
- **3.12 Atualizar Contadores**: Incrementar contagem de registros processados, totalizadores
...
- Gerar registro de rodapé (tipo 9) com metadados finais
...
Se cache de conversões (Redis, VSAM) estiver indisponível:
...
Se consulta a tabelas DB2 (VIP, VIPA, MCI) retornar erro ou timeout:
...
Se registro de entrada violou contrato esperado (formato inválido, tamanho incorreto):
...
Se registro de entrada contiver identificador de pessoa jurídica que não possua correspondência em base de dados interna (DB2VIP, DB2VIPA), o processamento pode resultar em rejeição silenciosa ou geração de registro incompleto.

**Impacto**

Perda de dados de portador; inconsistência entre entrada e saída; impossibilidade de enriquecer dados de cartão.
...
Se programa VIPP4365 ou VIPP4553 encerrar naturalmente (status 0 = sucesso), mas registro de rodapé indicar inconsistência de contagem, sistema pode gerar alerta falso de erro ou falha de integridade.

**Impacto**

Reprocessamento desnecessário; bloqueio de liberação de dados para downstream; alarme falso em operação.
...
### R03 — Ambiguidade Semântica não Detectada (Risco Crítico)

**Descrição**

Se campo de CNPJ não for classificado corretamente (mantendo ambiguidade entre Chave e Valor), sistema pode aplicar regra incorreta (ex.: validar DV em Chave de CNPJ ou expor Chave em interface externa).

**Impacto**

Violação de contrato técnico; violação de segurança e conformidade; corrupção de dados de negócio.

**Mitigação**

- Realizar análise de classificação para 100% dos campos identificados como CNPJ
- Documentar evidências técnicas de cada classificação
- Implementar validação de contrato em runtime para detectar usos indevidos
- Gerar relatório de campos com classificação Indefinido para análise manual

### R04 — Falha de Determinismo em Conversão (Risco Crítico)

**Descrição**

Se Serviço Central de Conversão (DFE) não garantir relação 1:1 obrigatória entre Chave de CNPJ e Valor de CNPJ, múltiplas conversões com mesmo input podem resultar em outputs diferentes.

**Impacto**

Perda de rastreabilidade; impossibilidade de auditoria; violação de integridade relacional.

**Mitigação**

- Implementar validação de determinismo (mesmo input → mesmo output)
- Utilizar apenas DFE como fonte única de verdade para correlações
- Proibir geração local ou distribuída de correlações
- Validar cache periodicamente contra DFE para detectar inconsistência

### R05 — Exposição Acidental de Chave de CNPJ (Risco Crítico)

**Descrição**

Se rotina de preparação de saída exibir Chave de CNPJ em arquivo de saída, relatório ou interface externa, violação de contrato técnico e segurança ocorre.

**Impacto**

Vazamento de identificador técnico interno; violação de segurança; conformidade comprometida.

**Mitigação**

- Implementar validação obrigatória de tipo de campo em cada escrita de saída
- Rejeitar tentativas de escrever campo numérico PIC 9(14) em interface externa
- Gerar alerta crítico se tentativa de exposição de Chave for detectada
- Incluir validação no esquema de output (layouts, copybooks)

### R06 — Falha de Integração com Serviço Central (Risco Alto)

**Descrição**

Se Serviço Central de Conversão (DFE) estiver indisponível durante processamento, sistema não consegue converter identificadores, bloqueando lote inteiro.

**Impacto**

Interrupção de processamento; atraso de saída de dados para downstream; impacto operacional.

**Mitigação**

- Implementar cache proativo de correlações (Redis, VSAM)
- Implementar retry automático com backoff exponencial
- Permitir fallback para conversão cached se DFE indisponível
- Gerar alerta de indisponibilidade para monitoramento proativo
- Implementar timeout controlado para evitar travamento

### R07 — Violação de Contrato de Dados Externos (Risco Médio)

**Descrição**

Se dados originários de fonte externa (DB2MCI.CLIENTE, DAF) forem transformados ou normalizados contra regra RF09, integridade de origem é comprometida.

**Impacto**

Perda de rastreabilidade de origem; impossibilidade de reconciliação com fonte; divergência de dados.

**Mitigação**

- Implementar flag de origem em cada campo (interna vs. externa)
- Bypassar pipeline de transformação para campos de origem externa
- Registrar em auditoria cada vez que campo externo é preservado intacto
- Validar em testes que dados externos nunca são transformados

### R08 — Regressão em Sistemas Legados (Risco Médio)

**Descrição**

Se modificações do Sistema VIP (VIPP4365, VIPP4553) introduzirem mudanças em contratos de dados que sistemas legados dependem, regressão pode ocorrer em downstream.

**Impacto**

Quebra de contrato técnico com sistemas consumidores; falha em downstream; bloqueio de liberação.

**Mitigação**

- Validar que mudanças são restritas a classificação semântica, não a tipo físico de dado
- Manter compatibilidade retroativa com layouts antigos
- Incluir versioning de contrato de saída
- Coordenar mudança com consumidores downstream

---

## Glossário

Tabela 8 — Termos técnicos, funcionais e conceituais utilizados no projeto, com definições que garantem consistência de vocabulário em discussões de classificação, conversão, integração e transformação de dados.
...
| **COMP-3** | Tipo de compactação COBOL (empacotado decimal, 2 dígitos por byte). Usado para campos numéricos em DB2 e arquivos compactados. |
| **PIC 9(14)** | Notação PIC (Picture) COBOL para campo numérico de 14 dígitos. Utilizado para Chave de CNPJ. |
| **PIC X(14)** | Notação PIC (Picture) COBOL para campo alfanumérico de 14 caracteres. Utilizado para Valor de CNPJ. |
| **Ambiente IBM z/OS** | Mainframe IBM OS. Compilador COBOL SOS 13. Suporta DB2, VSAM, JCL, CICS. Ambiente padrão do Sistema VIP. |
| **DB2** | Sistema gerenciador de banco de dados relacional em ambiente mainframe. Suporta schema, tabelas, índices, constraints. |
| **VSAM** | Virtual Storage Access Method. Método de acesso a arquivos em mainframe. Utilizado para datasets organizados e indexed. |
| **JCL** | Job Control Language. Linguagem de controle de jobs em mainframe. Define steps, parâmetros e alocações. |
| **Classificação NR/RC** | Nível de Risco (NR) e Risco de Crédito (RC). Classificação de risco de portador ou cartão, independente de formato de CNPJ. |
| **Filtro FLTR-AUTZ** | Filtro de autorização. Regra de negócio que determina elegibilidade de portador ou cartão conforme critérios predefinidos. |
